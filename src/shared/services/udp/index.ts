...
import { SOCKET_EVENTS, UDP_BROADCAST_ADDRESS, UDP_STATE } from './constants';
...
export class UdpService extends EventEmitter {
  ...
  constructor(port: number) {
    ...
    this.#socket.on(SOCKET_EVENTS.LISTENING, () => {
      ...
      this.emit(
        UDP_STATE.UDP_STATE_READY,
        `UDP socket ready on ${address.address}:${address.port}`
      );
    });
  ...
  protected handleSocketEvent = (data: string): void => {
    console.log(data);
  };
